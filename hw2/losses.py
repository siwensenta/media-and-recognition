...
class CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, label):
        ...
        losses = - torch.log(torch.sum((label_conv * q), dim = 1) + 1e-9)
        loss = torch.mean(losses)
        ctx.save_for_backward(q, label_conv)
