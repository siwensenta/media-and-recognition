...
class MyDataset(Dataset):
    def __init__(self, file_path):
        ...
        '''
        read_data = np.load(file_path)
        self.features = read_data[:,0:2].astype(np.float32)
        self.labels = read_data[:,-1].astype(np.longlong)
...
class Model(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.weight = nn.Parameter(torch.Tensor(output_size, input_size), requires_grad=True)
        self.bias = nn.Parameter(torch.Tensor(output_size), requires_grad=True)
    ...
    def forward(self, x):
        ...
        tmp = (torch.matmul(x, self.weight.T) + self.bias).squeeze()
        pred = self.active(tmp)
        return pred
        # pass
...
def bce_loss(pred, label):
    ...
    for idx in range(num):
        if label[idx]:
            loss -= torch.log(pred[idx])
        else:
            loss -= torch.log(1 - pred[idx])
    loss /= num
...
def train_val(train_file_path='data/character_classification/train_feat.npy',
    ...
    train_data = MyDataset(train_file_path)
    val_data = MyDataset(val_file_path)
    ...
    for epoch in range(n_epochs):
        ...
        model.train()
        # TODO: train the model for one epoch, which may include 
        # data loading, model forwarding, loss calculating, back propagation and parameters updating
        ...
        if (epoch + 1) % valInterval == 0:
            ...
            model.eval()
            # TODO: evaluate the model on the validation set, which may include 
            # data loading, model forwarding and accuracy calculating
            # remember to use torch.no_grad() because we do not need to compute gradients during validation
            N_correct = 0
            N_total = val_data.labels.shape[0]
            val_loss = 0
            with torch.no_grad():
...
def visual(model_path, file_path):
    ...
    with torch.no_grad():
        ...
        for feat, label in loader:
            ...
            if out.item() > 0.5:  # predicted character images
