...
class Conv2dFunction(Function):
    ...
    def forward(ctx, input, weight, bias, kernel_size, stride, padding):
        ...
        # TODO 1.1: calculate the height and width of output feature maps 
        # via input_h/input_w, kernel_h/kernel_w, stride_h/stride_w and padding_h/padding_w.
        ...
        # Please refer to the introduction of img2col in our lecture notes for this homework in week 6. 媒体与认知第三次习题课课件.pdf
        ...
        x_cols = nn.functional.unfold(input, kernel_size, stride, padding)
        
        # step 2: reshape weights and calculate W * x based on the unfolded x_cols, weight_re via torch.matmul()
        # reshape the size of weights from [out_channels, in_channels, kernel_h, kernel_w] 
        # to [out_channels, in_channels*kernel_h*kernel_w] (o, c)
        ...
        weight_re = weight.reshape([out_channels, in_channels*kernel_h*kernel_w])
        #print("weight_re : {}".format(weight_re.shape))
        ...
        output = torch.matmul(weight_re, x_cols) + bias_re
        output = output.reshape([batch_size, out_channels, out_h, out_w])
        # end TODO 1.2
        
        #print("x_cols : {}".format(x_cols.shape))
        #print("bias_re : {}".format(bias_re.shape))
    ...
    def backward(ctx, output_grad):
        ...
        # reshape the size of dL/dy from [batch_size, out_channels, out_h, out_w] 
        # to [batch_size, out_channels, out_h*out_w] (b,o,n)
        output_grad = output_grad.reshape([batch_size, out_channels, out_h*out_w])
        # reshape the size of weight from [out_channels, in_channels, kernel_h, kernel_w] 
        # to [out_channels, in_channels*kernel_h*kernel_w] (o,c)
        weight_re = weight.reshape([out_channels, in_channels * kernel_h * kernel_w])
        ...
        # the size of dL/d(x_cols) (i.e. x_cols_grad) is 
        # [batch_size, in_channels*kernel_h*kernel_w, out_h*out_w] (b,c,n)
        x_cols_grad = torch.matmul(weight_re.T, output_grad)
        # Use torch.nn.functional.fold function to transfer the size of x_cols_grad from 
        # [batch_size, in_channels*kernel_h*kernel_w, out_h*out_w] to [batch_size, in_channels, input_h, input_w].
        ...
        x_grad = nn.functional.fold(x_cols_grad, input_size, kernel_size, stride, padding)
        
        # end TODO 1.3
        
        
        #print("output_grad : {}".format(output_grad.shape))
        #print("weight_re : {}".format(weight_re.shape))
        #print("x_cols : {}".format(x_cols.shape))
        #print("W_grad_re : {}".format(W_grad_re.shape))
        #print("W_grad : {}".format(W_grad.shape))
        #print("b_grad : {}".format(b_grad.shape))
        #print("x_cols_grad : {}".format(x_cols_grad.shape))
        #print("x_grad : {}".format(x_grad.shape))
...
class Conv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        self.W = nn.Parameter(torch.randn(W_size), requires_grad=True)
        self.b = nn.Parameter(torch.randn(self.out_channels), requires_grad=True)
        # End TODO 1.4
