...
class CNN(nn.Module):
    def __init__(self, in_channels, num_class, batch_norm=False, p=0.0):
        ...
        self.conv1 = nn.Sequential(
            Conv2d(in_channels, out_channels=32, kernel_size=5, stride=1, padding=2),
            bn2d(num_features=32),
            nn.ReLU()
        )
        # conv       5          1        2          64
        # batchnorm
        ...
        # what is the size of output of convolution layers?
        # Network structure:
    ...
    def forward(self, x, return_features=False):
        ...
        x1 = self.conv1(x)
        x2 = self.conv2(x1)
        out_pool1 = self.pool1(x2)
        x3 = out_pool1  + self.conv3(out_pool1)
        x4 = self.conv4(x3)
        out_pool2 = self.pool2(x4)
        x5 = out_pool2 + self.conv5(out_pool2)
        # step 2: using Tensor.view() to flatten the tensor so as to match 
        # the size of input of fully connected layers. 
        x5_flatten = x5.view(batch_size, -1)
        # step 3: forward process for linear layers
        out = self.fc_net(x5_flatten)
        # end TODO 2.3
